...
class BitShift():
    ...
    def get_onnx(self):
        ...
        model = helper.make_model(helper.make_graph(
            ...
            name="test-model",
            inputs=[
                helper.make_tensor_value_info("x", TensorProto.UINT8, named_input),
                helper.make_tensor_value_info("y", TensorProto.UINT8, named_input),
            ],
            outputs=[
                helper.make_tensor_value_info("bitshifted", TensorProto.UINT8, named_input)
            ]
...
def test_bitshift():
    ...
    pred = sess.run([label_name], {input_name_1: x.astype(np.uint8), input_name_2: y.astype(np.uint8)})[0]
